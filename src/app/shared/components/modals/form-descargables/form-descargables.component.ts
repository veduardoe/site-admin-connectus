...
@Component({
  selector: 'app-form-descargables',
  templateUrl: './form-descargables.component.html',
  styleUrls: ['./form-descargables.component.scss']
})
export class FormDescargablesComponent implements OnInit {
  ...
  mainForm = new FormGroup({
    ...
    idCategoriaDescargable: new FormControl(null, [Validators.required]),
  ...
  save() {
    ...
    if (!data.fotoPresentacion) {
      this.utils.fnMainDialog("Error", "You must select all the pictures.", "message");
      return;
    }

    if (!data.fotoPresentacion) {
      this.utils.fnMainDialog("Error", "You must select the file.", "message");
      return;
    }
    ...
    const prom = (data._id) ? this.conDesService.putContent(data._id, data) : this.conDesService.postContent(data);
  ...
  prepareFile(file: any, id) {
    ...
    reader.onload = () => {
      ...
      if (id === 'fotoPresentacion' && mimeTypeImagenes) {
        this.mainForm.patchValue({ fotoPresentacion: fichero });
      } else if (id === 'fichero') {
        this.mainForm.patchValue({ fichero, ficherox: this.sanitizer.bypassSecurityTrustResourceUrl(fichero) });
      } else {
  ...
  removerImagen(id) {
    this.utils.fnMainDialog('Confirm', 'are you sure to remove?', 'confirm').subscribe(res => {
      if (res) {
        ...
        if (id === 'fichero') {
          this.mainForm.patchValue({ fichero: null });
        }
      }
    })
  }
